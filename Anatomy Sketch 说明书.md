...
图像分割是医学影像分析中的重要处理方法，Anatomy Sketch除了最基本的[阈值分割](#371-阈值分割)和[区域生长](#372-区域生长)外还提供了两种实际应用场景较多了两种交互式分割方法：[2D轮廓线插值](#373-2D轮廓线插值)和[智能分割（图割算法）](#374-智能分割（图割算法）)。
...
* [Model]  
  ...
  # Interface = DLL
  ```
  必须指定对应Python文件的名称(不包括后缀名)，该Python文件种应该包含入口函数，如：
  ```ini
